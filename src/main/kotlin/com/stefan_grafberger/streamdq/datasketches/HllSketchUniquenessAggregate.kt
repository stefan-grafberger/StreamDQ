...
class HllSketchUniquenessAggregate<T>(
    ...
    override fun getResult(hllSketchAggregation: Any): AggregateConstraintResult {
        hllSketchAggregation as HllSketch
        val (totalCount, estimate) = hllSketchAggregation.result
        val approxUniqueness = (BigDecimal(estimate) / BigDecimal(totalCount)).toDouble()
        val withinBounds = (expectedLowerBound == null || expectedLowerBound <= approxUniqueness) &&
            (expectedUpperBound == null || expectedUpperBound >= approxUniqueness)
        return AggregateConstraintResult(withinBounds, approxUniqueness, checkName)
    }
