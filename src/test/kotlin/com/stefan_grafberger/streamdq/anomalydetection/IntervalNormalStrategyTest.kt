...
class IntervalNormalStrategyTest {
    ...
    fun test(){
        ...
        val intervalS : IntervalNormalStrategy = IntervalNormalStrategy(lowerDeviationBound, upperDeviationBound, includeInterval)
        val list = intervalS.detect(data, searchInterval)

        Assertions.assertEquals(0, list.iterator().next().first)
        Assertions.assertEquals("[SimpleThresholdStrategy]: data value 1.0 is not in [1.433520605161735, 16.26591757935306]"
        , list.iterator().next().second.detail)
    }
}